#!/bin/bash
# Feishu voice chat - 使用微软 Edge TTS 发送飞书语音条
# 用法：bash send_voice.sh -t "文字内容" [--reply-to 消息ID] [-v 音色] [-r 语速] [-p 音调]
...
if [ "$NO_SEND" = false ]; then
    if [ -z "$FEISHU_APP_ID" ] || [ -z "$FEISHU_APP_SECRET" ]; then
        ...
        echo "  export FEISHU_CHAT_ID=\"oc_xxx\"  # 非 reply 模式需要"
        exit 1
    fi
fi
...
if [ -n "$REPLY_TO" ]; then
    RESULT=$(curl -s -X POST "https://open.feishu.cn/open-apis/im/v1/messages/$REPLY_TO/reply" \
      -H "Authorization: Bearer $TOKEN" \
      -H "Content-Type: application/json" \
      -d "{\"msg_type\":\"audio\",\"content\":\"{\\\"file_key\\\":\\\"$FILE_KEY\\\",\\\"duration\\\":$DURATION_MS}\"}")
else
    RESULT=$(curl -s -X POST "https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=chat_id" \
      -H "Authorization: Bearer $TOKEN" \
      -H "Content-Type: application/json" \
      -d "{\"receive_id\":\"$CHAT_ID\",\"msg_type\":\"audio\",\"content\":\"{\\\"file_key\\\":\\\"$FILE_KEY\\\",\\\"duration\\\":$DURATION_MS}\"}")
fi
